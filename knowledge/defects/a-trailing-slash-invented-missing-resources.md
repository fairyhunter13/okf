...
description: anyExists reads filepath.Dir(Root) as the repo root. So a root spelled knowledge/ resolved every repo-relative resource against the bundle itself. That gave 12 conformance errors on one real bundle with the slash, and 0 without.
...
needs a caller who types one: an audit, a shell completion, a `for` loop over `*/knowledge/`. The
fleet's own gates never do. So it was reachable by users and unreachable by the suite: coverage
...
`CheckBundleWith` and `Load` still clean the root. `anyExists` derives the repo base from
`filepath.Abs(d.Root)`, rather than from the string it was handed. That is correct for every
