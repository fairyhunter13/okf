...
A source that fails that test proves reachability and nothing more — which is all it had ever
...
tracker that stamps its own HTML with a request id, and it never will, because the page is not
...
They are the concept's provenance whether or not a digest can watch them. Dropping them to keep the
check quiet would trade a real citation for a green run, and a reader following the link is the
